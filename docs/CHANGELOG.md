...
## v4.1 - Headless Mode 测试与平衡调整版 (当前版本)

### 🔧 平衡调整 (Balance Adjustments)

#### 寒冷系统调整 (Cold System Tuning)
- **降低寒冷增长速率**:
  - 白天寒冷增长: `1.0` → `0.5` 每小时
  - 夜晚寒冷增长: `5.0` → `2.0` 每小时
- **降低寒冷伤害**:
  - 基础伤害概率: `0.1` → `0.05`
  - 伤害量: `2.0` → `1.0`
- **效果**: 春季时村民寒冷值保持在合理范围 (50-54), 不再达到危险的100

### ✅ AI系统改进 (AI System Improvements)

#### 任务生成优化 (Job Generation Optimization)
- **自动生成砍树任务**:
  - AI系统现在自动为树木创建 `chop` 任务
  - 保持最多10个可用砍树任务在队列中
  - 解决了之前只有初始5棵树有任务的问题
- **任务生成节流**:
  - 任务生成频率限制为每10个tick一次
...
#### 食物寻找改进 (Food Finding Improvements)
- **优先查找仓库食物**:
  - 改进 `_find_and_eat_food` 逻辑
  - 优先选择仓库区域内的食物 (距离权重减半)
  - 扩大搜索范围至30格
  - 解决了村民无法找到仓库中食物的问题

### 📊 Headless Mode 增强 (Headless Mode Enhancements)

#### 日志系统扩展 (Logging System Extensions)
- **详细状态日志**:
  - 每1秒记录所有村民的完整状态
  - 包括: 位置、需求值 (饥饿/疲劳/心情/寒冷)、当前工作、库存、技能
  - 显示游戏时间、季节、可用任务数量
  - 显示仓库物品统计、剩余树木数量、作物状态
- **运行时长控制**:
  - Headless模式现在运行指定的游戏天数 (默认2天)
  - 基于游戏时间而非真实时间退出
  - 显示最终游戏时间和状态

### 🐛 Bug修复 (Bug Fixes)
- **修复任务生成循环**: 防止同一物品重复创建搬运任务
- **修复食物查找**: 村民现在能正确找到并食用仓库中的食物
- **修复任务分配**: 确保有足够的工作任务供村民选择

### 📝 测试结果 (Test Results)
经过2天游戏时间的headless模式测试:
- ✅ 村民正常工作 (砍树、搬运、收获、进食)
- ✅ 需求系统平衡 (饥饿、疲劳、寒冷在合理范围)
- ✅ 任务系统稳定 (41个可用任务, 无循环)
- ✅ 资源管理正常 (仓库有食物和种子, 树木持续被砍伐)
- ✅ 寒冷系统平衡 (春季寒冷值50-54, 不再达到危险水平)

### 📝 配置变更 (Configuration Changes)
- `config/balance.json`:
  - `entities.villager.needs.cold_gain_per_hour_day`: `1.0` → `0.5`
  - `entities.villager.needs.cold_gain_per_hour_night`: `5.0` → `2.0`
  - `entities.villager.needs.cold_damage_probability_base`: `0.1` → `0.05`
  - `entities.villager.needs.cold_damage_amount`: `2.0` → `1.0`
